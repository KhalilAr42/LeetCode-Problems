...
public class PalindromeNumber {
    ...
    public static boolean isPalindromNumberVersion3(int number) {
         ...
         * This method can cause a problem if the reverse of the number is superior than
         * int.
        ...
        // Since a number don't start with 0 every number that finish with 0 is not a
        // palindrome (except the number 0 itself)
    ...
    public static boolean isPalindromNumberVersion4(int number) {
         ...
         * In this version we will only go as far as the half of the number (like
         * version 2 using string)
         * But how do we know when we reached half of the number
         * Since we divide each time by 10, once our reversed_number variable >
         * original_number
         * We are sure that we exceeded or reached the middle of the number
         * If the number have an odd number of digit we can just get rid of the extra
         * one since it's equal to itself
         * Time complexity(O(half_number_of_digits)) = O(log10(n)/2) = O(log10(n)) ,
